...
function ode_function_unthreaded!(fwm, du, u, ps, t)
...
function ode_function(fwm, threaded)

    if threaded

        du_locks = [ReentrantLock() for i in 1:(length ∘ variables)(fwm.vars)];
        return (du, u, ps, t) -> ode_function_threaded!(fwm, du_locks, du, u, ps, t)
    end

    return (du, u, ps, t) -> ode_function_unthreaded!(fwm, du, u, ps, t)
end
